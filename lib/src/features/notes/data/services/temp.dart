...
class Note {
  ...
  String title;
  String content;
  String date; // Store as ISO string for sorting
  bool isBookmarked;
  bool isArchived;
...
class NotesDatabase {
  ...
  Future<List<Note>> getNotes({
    ...
    final result =
        await db.query('notes', where: whereClause, whereArgs: whereArgs, orderBy: orderBy);
  ...
  Future<int> archiveNote(int id) async {
    final db = await instance.database;
    return await db.update('notes', {'isArchived': 1}, where: 'id = ?', whereArgs: [id]);
  }

  // Restore archived note
  Future<int> restoreNote(int id) async {
    final db = await instance.database;
    return await db.update('notes', {'isArchived': 0}, where: 'id = ?', whereArgs: [id]);
  }

  // Get archived notes
  Future<List<Note>> getArchivedNotes() async {
    final db = await instance.database;
    final result = await db.query('notes', where: 'isArchived = 1', orderBy: 'date DESC');
    return result.map((e) => Note.fromMap(e)).toList();
  }
